...
class ImageCropper(QMainWindow):
    ...
    def on_mouse_move_event(self, event):
        if self.dragging:
            ...
            rect = QRect(self.start_drag_pos.x()-1, self.start_drag_pos.y()-1,896+2,512+2)
...
# new_img = Image.newcrop('RGB', (new_width, new_height), (255, 255, 255))
...
def denoise_oct_image(img):
    # 小波分解处理
    coeffs = pywt.wavedec2(img, wavelet=wavelet_name, level=2)

    # 去除细节系数
    coeffs_denoised = [coeffs[0]]
    for i in range(1, len(coeffs)):
        coeff_arr = coeffs[i]
        if isinstance(coeff_arr, tuple):
            #(cH, cV, cD)
            coeffs_denoised.append((np.zeros_like(coeff_arr[0]),
                                    np.zeros_like(coeff_arr[1]),
                                    np.zeros_like(coeff_arr[2])))
        else:
            coeffs_denoised.append(np.zeros_like(coeff_arr))

    # 逆小波变换处理
    img_denoised = pywt.waverec2(coeffs_denoised, wavelet=wavelet_name)

    # 转换为8位灰度图并返回结果
    img_denoised_uint8 = np.clip(img_denoised, 0, 255).astype(np.uint8)
    return img_denoised_uint8


# for i in os.listdir('./newcrop'):
#     path=os.path.join('./newcrop',i)
#
#     img=cv2.imread(path)
#     # 对图像进行降噪处理
#     img_denoised = denoise_oct_image(img)
#
#     # 显示结果
